!------------------------------------!
! Author Z.Hawkhead                  !
! Master File for serial execuatable !
!------------------------------------!
program Mandelbrot
  use files
  use fractal
  ...
  integer:: ierr, nprocs, rank=0, rem 
  ...
  integer :: d_t(8)
  character*10 :: b(3)
  character(len=100)::version, compiler,arch_string, DATE,TIME,comms="Serial"
  character(len=81)::parser_version="Mandelbrot v.2.1, Z.Hawkhead"
  character(len=100)::info="Serial code for calculating the Mandelbrot Set"
